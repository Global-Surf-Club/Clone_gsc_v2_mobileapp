...
const CreateReport = () => {
  ...
  const submitReport = async () => {
    ...
    try {
      ...
      const report = await Trip.postReport(data);
      const imageData = new FormData();
      for (let i = 0; i < imageList.length; i++) {
        const item = imageList[i];
        imageData.append(i, item);
      }
      const imageRes = await Trip.postReportImage(report?.id, imageData);
      setSuccess(true);
    } catch (error) {}
  ...
  const toAddress =
    (destinationAddress?.address1 ? destinationAddress?.address1 : '') +
    ' ' +
        ...
        <View style={styles.viewContainer}>
          ...
          <View>
            ...
            <View style={[styles.aiiiconrow]}>
              ...
              >
                <Text style={[styles.InformationHeading, { width: '100%' }]}>
          ...
          {/* <Pressable
                            ...
                            style={[styles.inputcontainer, styles.mt2, styles.aiiiconrow]} >
                            <TouchableOpacity style={styles.vfbtn2}>
                                <Text style={styles.vfbtntext}>VF</Text>
                            </TouchableOpacity>
                            <Text style={styles.subtitle}>Add A Surf Spot To This Trip</Text>
                        </Pressable> */}
          <View>
          ...
          <View>
            <View style={[styles.inputcontainer, styles.mt2, { width: '70%' }]}>
              <View style={styles.raitingrow}>
                <Text style={[styles.ratingtext, styles.pr10]}>Rating</Text>
                <View style={styles.raitingrow}>
                  {[0, 1, 2, 3, 4].map(i => {
          ...
          <View>
            ...
            >
              {verifiedForests.map((item, id) => {
                ...
                // const forecast =
                //   (forecastByDay[moment(item?.forecastDate).format('l')] ??
                //     [])[0] ?? {};
                // const direction = getWindDirection(
                //   Number(forecast?.windDirection?.sg ?? 0),
                // );
                return (
                  <Pressable
                    key={id}
          ...
          <View style={styles.buttoncontainer}>
            <BorderButton
              isProcessing={success || showLoading}
