...
const TripComments = () => {
  ...
  const reduxComments = useSelector(state => state.trip.comments);
      ...
      if (routeItem?.ListIndex === -1) {
        ...
        if (parseInt(routeItem?.ListIndex) > 10) {
          ...
          if (modules == 0) {
            ...
            currentPage.current = pageNo.current;
      ...
      getComments(
        ...
        (success, hasMore) => {
          ...
          if (success) {
            if (loadMore) {
              pageNoBottom.current += 1;
              currentPage.current = pageNoBottom.current;
              hasMoreComments.current = hasMore === true;
            } else if (isTop) {
              pageNoTop.current -= 1;
              currentPage.current = pageNoTop.current;
  ...
  const handleSendComment = async () => {
    if (!inputComment.trim() || isSendInProgress || !tripId) {
      return;
    }
    ...
    try {
      ...
      await dispatch(commentTripAction(tripId, commentText, user));
    } catch (error) {
    ...
    <SafeAreaView style={styles.container}>
      ...
      >
        <Header
          ...
          title={routeItem?.isClub ? 'Club Trip Comments' : 'Trip Comments'}
          notification={'6'}
          textAlign={'center'}
        />
          ...
          >
            {isSendInProgress ? (
              ...
              <FontAwesome
                name="send"
                size={20}
                color={inputComment.trim() ? Color.themeColor : Color.cardgray}
              />
            )}
          </Pressable>
        </View>
