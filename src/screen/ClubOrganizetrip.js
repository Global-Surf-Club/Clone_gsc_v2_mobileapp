...
const ClubOrganizetrip = props => {
  ...
  // const regions = useSelector(state => state?.trip?.regions);
  const regions = useSelector(state => state?.trip?.ModifiedResons);
  const [searchLoader, setSearchLoader] = useState(false);
  const [searchedData, setSearchedData] = useState(null);
  // const spot = useSelector(state => state?.trip?.spot);
    ...
    if (currentRegion?.id) {
      if (currentRegion?.spot?.id) {
        if (route?.params?.isEdit) {
          ...
          navigation.navigate('ClubOrganizeForm', { clubid });
        }
      } else {
        alert('No spot found');
      }
              ...
              >
                {filteredData.map(item => {
                  return (
                    <Region
                      item={item}
                      searchQuery={searchLoader ? '' : searchQuery}
                    />
                  );
                })}
              </List.Section>
...
export default ClubOrganizetrip;
