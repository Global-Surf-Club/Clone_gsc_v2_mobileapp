...
import { TripReport } from '../components/CommunityItems';
...
import { checkOnlineStatus } from '../database/profileDbHelper';
...
const TripDetail = () => {
  ...
  const isOnline = useSelector(state => state.trip.isOnline);
  ...
  const newIdForForecast = `${routeItem?.destination?.id}_${
  ...
  const [selectedImages, setSelectedImages] = useState([]); // {uri, base64, tag}
  ...
  useEffect(() => {
    setTripLoader(true);
    dispatch(
      getCurrentTrip(routeItem?.id || routeItem?.tripId, status => {
        setTripLoader(false);
      }),
    );
    getPendingUsers();
    getInterstedUsers();
    getPassenger();
    if (routeItem?.id || routeItem?.tripId) {
  ...
  const getPassenger = () => {
    ...
    dispatch(
      getPassengers(routeItem?.id || routeItem?.tripId, () => {
  ...
  const getInterstedUsers = () => {
    ...
    dispatch(
      getInterstedUser(routeItem?.id || routeItem?.tripId, () => {
  ...
  const getPendingUsers = () => {
    ...
    dispatch(
      getPendingList(routeItem?.id || routeItem?.tripId, () => {
  ...
  const removePassenger = async (id, uId) => {
    Alert.alert(
      'Alert',
      uId == user?.id
        ? 'Are sure to leave trip'
        : 'Are you sure to remove passenger',
      [
        {
          text: 'Cancel',
        },
  ...
  const GotoInvite = () => {
    if (isTripStarted) {
      alert('You can not update crew after 6 hr of trip started');
      return;
    }
    navigation.navigate('Invite', { clubid: '-999' });
  };
  ...
  const checkInTripFinal = async (lat, long) => {
    try {
      ...
      const data = JSON.parse(
        await Trip.checkInTrip(routeItem?.id || routeItem?.tripId, lat, long),
      );
      if (data) {
  ...
  const handleTextChange = text => {
    ...
    // ✅ Step 2: Handle mention suggestions
  ...
  const loadChats = async (
    ...
    console.log('perPageRef.current ===>', perPageRef.current);
  ...
  const handleSend = async () => {
    try {
      ...
      const requestData = selectedImages.map(img => ({
        tripId: routeItem?.id || routeItem?.tripId,
        text: img.tag || '',
        ClubId: -999,
  ...
  const sendChat = async () => {
    ...
    try {
      ...
      const tripId = routeItem?.id || routeItem?.tripId;
  ...
  // ---------- Paste these helpers INSIDE your component (before return) ----------
  const getLoaderForSelectedIndex = () => {
    switch (selectedIndex) {
      ...
      case 1: // Forecast
        return !!forecastLoader || !!tripLoader || !!isInitialLoading;
      case 2: // Crew
        return (
          !!passengerLoader ||
          !!pendingListLoader ||
      ...
      case 3: // Expenses
        return !!expensesLoader || !!isInitialLoading || !!TopLodaerExpense;
      case 5: // Report
        return !!reporeLoader || !!bottomLodaerReport || !!TopLodaer;
      case 6: // Chat
        return !!chatLoader || !!isInitialLoading || !!refreshLoader;
      default:
        return !!showLoading;
    }
  };

  const isRefreshingForSelectedIndex = () => {
    // used by RefreshControl 'refreshing' prop for each ScrollView
    switch (selectedIndex) {
      case 0:
        return (
          !!tripLoader ||
      ...
      case 2:
        return (
          !!passengerLoader || !!pendingListLoader || !!interestedUsersLoader
        );
      case 3:
      ...
      case 5:
        return !!reporeLoader;
      case 6:
        // chat uses its own refreshLoader in FlatList; keep that as primary
        return !!refreshLoader || !!chatLoader;
      default:
        return !!showLoading;
    }
  };

  const onRefreshByTab = async () => {
    try {
      if (selectedIndex === 0) {
        if (typeof getPassenger === 'function') await getPassenger();
        if (typeof getInterstedUsers === 'function') await getInterstedUsers();
        if (typeof getPendingUsers === 'function') await getPendingUsers();
        if (typeof getPassenger === 'function') await getPassenger();
        if (typeof refreshData === 'function') await refreshData();
      } else if (selectedIndex === 1) {
        if (typeof getForecase === 'function') await getForecase();
        if (typeof refreshData === 'function') await refreshData();
        ...
        if (typeof getExpense === 'function') await getExpense(true);
      } else if (selectedIndex === 5) {
        if (typeof getTripReport === 'function') await getTripReport(true);
      } else if (selectedIndex === 6) {
        if (typeof loadChats === 'function')
          await loadChats(1, false, false, true);
      } else {
        if (typeof refreshData === 'function') await refreshData();
      }
    } catch (err) {}
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      >
        ...
        <Loader removeModal visible={getLoaderForSelectedIndex()} />
            ...
            <Pressable
              onPress={() => {
                ...
                if (
                  ...
                  alert('You can not update trip after started');
                  return;
                }
                navigation.navigate('OrganizeForm', { item: trip });
              }}
        ...
        <View style={styles.viewContainer}>
          {selectedIndex === 0 && (
            <ScrollView
              ...
              refreshControl={
                <RefreshControl
                  refreshing={isRefreshingForSelectedIndex()}
                  onRefresh={onRefreshByTab}
                />
              }
            ...
            >
              <View style={styles.detailsection2}>
                ...
                <View style={[styles.flexrow, styles.ph6]}>
                  <Text
                    style={[styles.nametext, { flex: 0, flexWrap: 'nowrap' }]}
                  >
                    {'Spot:'}
                  </Text>
            ...
            <>
              ...
              >
                <View style={styles.detailsection2}>
                            ...
                            !isUserDeleted && (
                              <Entypo
                                onPress={() => {
                                  if (isTripStarted) {
                                    alert(
                                      'You can not update crew after 6 hr of trip started',
                                    );
                                    return;
                                  }
                                  removePassenger(
                  ...
                  {/* interested, pending sections ... (unchanged) */}
                  <View style={[styles.ph6, styles.mt2]}>
                    <View style={[styles.iconflex]}>
                      <MaterialIcons
                        name="remove-red-eye"
                ...
                {/* optional Invite button unchanged */}
              </ScrollView>
