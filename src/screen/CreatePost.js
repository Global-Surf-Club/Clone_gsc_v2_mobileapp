...
import { createForumPost, updateForumPostThunk } from '../store/forumSlice';
...
const CreatePost = props => {
  const { ForumDetails } = props?.route?.params;
      ...
      dispatch(
        updateForumPostThunk(
          ...
          (success, message) => {
            ...
            if (success) {
              setSuccess(true);
              setSuccessDescription(message || 'Forum updated successfully');
            } else {
              setIserror(true);
              setSuccess(true);
              setSuccessDescription(message || 'Failed to update forum');
            }
          },
        ),
      );
  ...
  const getPostDetail = async postId => {
    try {
      ...
      if (data) {
        setImageUrlList(data?.postImages);
      }
    } catch (error) {
  ...
  return (
    <SafeAreaView style={styles.container}>
      <Header
        ...
        notification={'6'}
        title={ForumDetails ? 'Update Forum' : 'Create Forum'}
        textAlign={'center'}
      />
