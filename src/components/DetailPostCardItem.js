...
import { toggleForumPostLike } from '../store/forumSlice';
...
const DetailPostCard = props => {
  ...
  useEffect(() => {
    // keep local state in sync when item changes from parent/redux
    ...
    >
      {user?.id === item?.createdBy ? (
        <ClubmultiplePopover
          ...
          onPressDelete={() => onPreaaDeleteFourm(item?.id)}
        />
      ) : (
        <></>
      )}
      ...
      <>
        <View style={styles.mt2}>
          ...
          <Text style={styles.tescriptiontext}>{item?.description}</Text>
        </View>
        {item?.postImages?.length > 0 && (
          <View style={{ marginVertical: 10 }}>
            ...
            <ListModal
              ...
              visible={reportModal}
              onPressReport={() => {
                blockreport(selectusersID, 'forumimage', 'report', 'Reported');
              }}
            />
            <Pagination
            ...
            <View style={styles.iconbtnrow}>
              <Pressable
                ...
                onPress={async () => {
                  ...
                  dispatch(
                    toggleForumPostLike(item?.id, prevLiked, success => {
                      if (!success) {
              ...
              <Pressable style={styles.iconbtn} onPress={onclickcomment}>
                <Text style={styles.nametext}>
...
export default memo(DetailPostCard);
